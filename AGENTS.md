...
- 每个索引节点只链接自己的下一层节点；下一层可以是更细的中间节点或具体笔记

### 7.2 层级深度
- 文件系统默认只有 **2 层**：大类目录 → 笔记文件
- 图谱默认使用 **3 层节点**：大类索引 → 中间层级节点 → 具体笔记
- 如果中间层级下面还有语义明确的分组（如 `Core Language`、`Completion`），该分组必须建立带 ID 的独立节点，不能只写成普通标题
- 普通标题只用于单篇笔记内部的内容排版，不能代替图谱中的知识层级节点
- 图谱最多 4 层：大类索引 → 中间层级节点 → 子分组节点 → 具体笔记
...
- 中间层级索引中凡是代表分类的标题，也必须提升为独立 org-roam 节点；索引中不保留“只有标题、标题下挂链接”的隐式层级
...
- 节点的 `#+title` 必须带足领域上下文，避免全库出现 `Core Language`、`Development Tools`、`Data Structures` 等过于通用、容易跨大类重名的标题
- 上级索引中的链接描述可以保持简短，例如目标节点标题为 `Emacs Lisp Core Language`，链接可写为 `[[id:...][Core Language]]`
...
- 如果新增的中间层级下面仍有分类层次，继续为这些分类建立带 ID 的子分组节点
- 除 `Inbox/` 中的未分类笔记外，不允许具体笔记或中间节点脱离从大类索引开始的索引链
...
### 7.11 图谱骨架与节点出度规则
- 大类索引、中间层级索引和子分组索引共同形成一棵**主索引树**；正文中的知识引用和反链可以自由形成网状关系
- 除大类根节点外，每个索引节点和具体笔记在主索引树中默认只有一个**主父节点**，避免重复收录导致图谱骨架含混
- 如果一篇笔记确实跨多个领域，只在最合适的位置作为主归属；其他位置通过正文中的语义链接引用，不重复充当主索引入口
- 索引必须逐层连接，不允许大类节点绕过已有中间节点直接链接叶子，也不允许中间节点绕过已有子分组直接链接叶子
- 单个索引节点建议连接 **2–8 个**下一层节点；超过 8 个时应评估拆分，超过 10 个时原则上必须增加语义明确的中间节点
- 只有一个子节点的中间节点通常应合并；如果它代表稳定的领域边界、资源类型或预期会继续扩展，可以保留
- 控制节点出度应依靠语义中间节点，不依靠增加文件系统目录

### 7.12 重构与验证规则
- 只要节点语义没有变化，移动文件、调整层级或修改标题时必须保留原 `:ID:`；只有创建全新概念节点时才生成新的 UUID v4
- 删除或合并旧索引节点前，先检查整个知识库中的入链；仍有有效入链时，应更新引用或保留节点身份
- 重构完成后逐项验证：
  1. 大类根节点的直接出度是否合理，是否只连接下一层索引节点
  2. 所有中间节点和具体笔记是否都能从大类根节点沿主索引树到达
  3. 每个节点是否恰好有一个主父节点，是否存在遗漏收录或重复收录
  4. 全库 `#+title` 是否具有足够上下文，是否存在不必要的重名
  5. 所有 `file:` 资源链接在移动后是否仍指向存在的文件
  6. 普通笔记是否满足 TOC 规则，Org block、property drawer 和链接语法是否有效
  7. 文件系统是否仍保持大类内扁平，仅保留允许的资源目录
- 完成文件移动或 ID 索引调整后，执行一次 `org-roam-db-sync` 刷新数据库
