...
* Mounting/Umounting
Related files (format described in the ~fstab(5)~ manual page):
- =/proc/mounts=: a list of the currently mounted file systems
- =/etc/mtab=: ~mount/umount~ commands automatically maintain this file which including mount options
- =/etc/fstab=: maintained manually by the system administrator, contains descriptions of all of the available file systems.

** Stacking Mounts on the Same Mount Point
- Purpose:
  - to stack a new mount on an existing mount point *that is busy*.
  - is useful when we use the /tmpfs/ file system.
#+begin_src bash
mount /dev/sda4 /testfs
mount /dev/sda5 /testfs # stack a second mount on /testfs
mount | grep testfs
# /dev/sda4 on /testfs type ext4 (rw)
# /dev/sda5 on /testfs type reiserfs (rw)
umount /testfs # pop a mount from /testfs
mount | grep testfs
# /dev/sda4 on /testfs type ext4 (rw)
#+end_src
...
* System Calls
- ~mount/umount~
- ~statvfs/fstatvfs~: obtain information about a file system.
- File Attributes
  - ~stat/lstat/fstat~: retrieve information about a file, mostly *drawn from the file i-node*.
  - ~utime/utimes/utimensat/futimens~: modify file timestamps


* Other Details
- [[id:52cc876c-a7de-45de-a3d8-c28b12c08f78][tmpfs]]
