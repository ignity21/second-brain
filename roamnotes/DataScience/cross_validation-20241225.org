...
ridge = Ridge()
tuning = GridSearchCV(ridge, param_grid, cv=kf)
tuning.fit(X_train, y_train)
print(tuning.best_params_, ridge_cv.best_score_)
# {'alpha': 0.0001, 'solver': 'sag'} 0.75839

# Evaluate on test data
test_score = tuning.score(X_test, y_test)
print(f"Tuned parameters: {tuning.best_params_}, Accuracy on test data: {test_score}")
#+end_src
...
logreg_cv = RandomizedSearchCV(logreg, params, cv=kf, scoring='recall')
