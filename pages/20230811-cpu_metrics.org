:PROPERTIES:
:ID:       00e92d55-5469-4d5d-9566-e889d0b86ab3
:END:
#+title: CPU Metrics
#+filetags: :Linux:Metric:OS:

* Load Average
:PROPERTIES:
:ID:       fc75e5c3-3b88-44cf-bcaf-ed537a1a98ee
:END:
平均负载是指单位时间内，系统处于可运行状态和不可中断状态的平均进程数，也就是平均活跃进程数。
- running or runnable (on run queue): 可运行状态的进程是指正在使用 CPU 或者正在等待 CPU 的进程，状态码 R
- uninterruptible sleep (usually IO): 不可中断状态的进程则是正处于内核态关键流程中的进程，状态码 D
- top计测值：1分钟，5分钟，15分钟

** 高平均负载的几种情况
1. 高 CPU 使用率 -> 计算密集
2. 高 iowait -> IO 密集

* CPU Usage
:PROPERTIES:
:ID:       bf6dd008-dd56-4b67-9b78-cc6af75e9249
:END:
CPU 使用率，就是除了空闲时间外的其他时间占总 CPU 时间的百分比。

- =/proc/stat= 提供 CPU 和任务统计信息。CPU 使用率基于此数据算出。
#+begin_verse
  单位 10ms 即 100 USER_HZ

  cpu  100898 490 31309 2195013 10269 0 9757 0 0 0
  cpu0 13066 99 4666 273267 1044 0 4636 0 0 0
  cpu1 12794 48 4203 274005 1114 0 2619 0 0 0
  cpu2 12809 86 3815 274503 1218 0 1262 0 0 0
  cpu3 12629 33 3708 275046 993 0 576 0 0 0
  cpu4 12608 67 3700 274824 1158 0 220 0 0 0
  cpu5 12046 36 3833 274827 1385 0 182 0 0 0
  cpu6 12547 55 3753 274612 1438 0 147 0 0 0
  cpu7 12395 63 3629 273926 1915 0 114 0 0 0
#+end_verse
- ~man 5 proc~ 查找 =/proc/stat= 查看每列所表示的含义
- user, nice, system, idle, iowait, irq(interrupts), softirq, steal, guest, guest_nice
  - nice: 代表低优先级用户态 CPU 时间，也就是进程的 nice 值被调整为 1-19 之间时的 CPU 时间； nice 可取值范围是 -20 到 19，数值越大，优先级反而越低
  - iowait: 代表等待 IO 的 CPU 时间
  - irq: 代表处理硬中断的 CPU 时间
  - steal: 代表当系统运行在虚拟机中的时候，被其他虚拟机占用的 CPU 时间
  - guest: 运行虚拟机的 CPU 时间

** 计算公式
=/proc/stat= 统计的是开机以来的时间片数。为了计算 CPU 使用率，性能工具一般都会取间隔一段时间（比如 3 秒）的两次值，作差后，再计算出这段时间内的平均 CPU 使用率。
$$usage=1 - \frac{idle_t-idle_{t-1}}{total_t-total_{t-1}}$$

** 常用工具
- [[id:14101e01-aaa1-4a4c-af6d-c4738cdbeb93][~top~]]
- [[id:4baf76c7-7bd5-4bf4-b573-86b0dabd4869][~pidstat~]]
  - ~pidstat 1 5~: 采样间隔1秒，采样5次
- [[id:894e26d6-8bae-49ab-ac0c-e9812dd08d9b][~perf~]]: 定位导致 CPU 使用率高的工具

* [[id:706d7e91-8a8f-466f-b871-602e2efcd187][Context Switching]]
* Cache Hit Rate
:PROPERTIES:
:ID:       397c168b-2de6-419b-b588-c3dc860c164b
:END:
