...
- can be reassigned to another memory block on the heap

In order to benefit from call-by-reference/pointer, the size of the data type passed to the function has to surpass the size of the pointer on the respective architecture (i.e. 32 bit or 64 bit).
