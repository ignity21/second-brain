...
- Category type saves memory: ~series.nbytes~ or ~df.memory_usage()~ or just ~df.info()~

* Converting to Categorical Data
** String to Category
Common ways:
- ~series.replace({...})~ then ~astype('category')~
  - ~series.map({...})~ then ~astype('category')~
- ~category.cat.remove_categories~
#+begin_src python
# 0        19h
# 1     5h 25m
# 2     4h 45m
...
- Other Approach: Replace strings, then convert to ~Categorical~
