...
- Use bootstrap distribution to calculate SE
- z-score: \(z=\frac{SampStat-HypothValue}{StdError}\)
  - *Standardized Value*
  - Then can utilize the z-table (standard normal distribution table)
  - [[id:e671c7ff-b03d-47a3-aa8a-91277e0e80c3][Z-Distribution]]
- z-score -> p-value:
  - left-tailed: ~scipy.stats.norm.cdf(z_score, loc=0, scale=1)~
  - right-tailed: ~1 - norm.cdf(...)~

** Left-tailed
- Null hypothesis: \(H_{0}\): \(\mu=\mu_{0}\)
- Alternate hypothesis: \(H_{A}\): \(\mu<\mu_{0}\)
- Decision Criteria: reject the \(H_{0}\) if \(z < -z_{\alpha}\)
...
- Decision Criteria: reject the \(H_{0}\) if \(z < -z_{\alpha/2}\) or \(z > z_{\alpha/2}\)
- [[https://www.mathandstatistics.com/learn-stats/hypothesis-testing/two-tailed-z-test-hypothesis-test-by-hand][Example]]
...
- \(H_{0}\): The mean annual compensation of the population of data scientists is $110000
- \(H_{A}\): \(>110000\)
...
upper = np.quantile(boot_distn, 0.975)
# or
lower, upper = np.quantile(boot_distn, [0.025, 0.975])
...
# p-value (right-tailed) alpha = 0.05
p_value = 1 - stats.norm.cdf(z_score)
# p = 3.149874e-05 < 0.05, reject the null hypothesis

# Prefered: or use z critical value
z_critical = stats.norm.ppf(1 - alhpa)
# z-score > 1.64485, reject the null hypothesis
#+end_src
